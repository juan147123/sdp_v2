<template>
    ...
    <Dialog header="Checklist" :visible="modalcheckVisible" :closable="false" :draggable="false"
        pt:mask:class="backdrop-blur-sm" maximizable modal :style="{
            width: '70rem',
        }">
        <div class="grid">
            <div class="col-3">
                <Listbox v-model="selectedArea" :options="checkListArea" optionLabel="descripcion"
                    class="h-full md:w-14rem" />
            </div>
            <div class="col-9">
                <div class="flex justify-content-between flex-wrap w-full" v-if="selectedArea.length != 0">
                    <span class="font-bold white-space-nowrap">
                        <label for=""></label>Checklist
                    </span>
                    <Tag :severity="checklistSolicitud?.status == 1
                        ? 'secondary'
                        : 'success'
                        " :value="checklistSolicitud?.status == 1
                            ? 'Pendiente'
                            : 'Cerrado'
                            " class="mr-3"></Tag>
                </div>
                <form @submit.prevent="submit">
                    <div class="w-full contenedor-checklist pl-5 pr-5" v-for="(checkList, index) in this.selectedArea
                        .configuracion">
                        ...
                        <div class="grid">
                            <div class="col-8">
                                <div class="mb-2">
                                    ...
                                    <div class="flex gap-3 ml-3">
                                        <div class="flex align-items-center">
                                            <RadioButton v-model="formData[checkList.id]" :value="true" :disabled="formData.aplica != true ||
                                                checklistSolicitud?.status == 0
                                                " />
                                            <label class="ml-2">Sí</label>
                                        </div>
                                        <div class="flex align-items-center">
                                            <RadioButton v-model="formData[checkList.id]" :value="false" :disabled="formData.aplica != true ||
                                                checklistSolicitud?.status == 0
                                                " />
                                ...
                                <div class="mb-2 flex flex-column ml-3" v-if="checkList.input != 0">
                                    <label class="mb-2">Indique un monto</label>
                                    <InputText placeholder="" v-model="formData[
                    ...
                    <div class="flex justify-content-end gap-2 mt-5" v-if="selectedArea.length != 0">
        ...
        buildFormdata() {
            ...
            for (const checkList of this.selectedArea.configuracion) {
                ...
                const data = {
                    ...
                    [`${descripcion[0]}${id}`]: this.formData[`${descripcion[0]}${id}`] || null,
                    [`${id}`]: this.formData[`${id}`] || null,
                    [`checkarea${descripcion[0]}${id}`]: this.formData[`checkarea${descripcion[0]}${id}`] || false,
                    [`checklider${descripcion[0]}${id}`]: this.formData[`checklider${descripcion[0]}${id}`] || false,
                };
